...
class WuzzufProvider(JobProvider):
    ...
    def _parse_listing_page(self, html):
        ...
        for link in soup.find_all("a", href=re.compile(r"/jobs/p/")):
            ...
            job.update(exp_info)
            # استخراج عدد الأيام من تاريخ النشر
    ...
    def _extract_experience(self, card_text: str) -> dict:
        ...
        # لو معرفناش، نعتبر الخبرة عالية عشان نستبعدها
    ...
    # الدالتين التاليتين موجودتين في base.py، لكن نضيفهم هنا للتأكد
    ...
    def _experience_matches(self, job):
        ...
        if min_exp is None:
            return False  # نستبعد اللي مش عارفين خبرتهم
